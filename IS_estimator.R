...
calculate_mseparameters<-function(loops, different_expcases, M, states_dim, taus ){
  ...
  # loops=5;  different_expcases=different_cases;M=200;states_dim=p;taus=taus
  ...
  for(m in 1:M){
    ...
    acts_mat=actions_dgp_Xiong(taus = taus,ndays = Ndays_loop, ti=TI_loop )
    ...
    ##---estimating ATE with DIM estimator----
...
ggplot(data_ate_mse, aes(x=n,y=RMSE, color=m) )+geom_point(size=5)+geom_line(size=2)+
  facet_grid(.~rho0)+
  theme_bw()+
  ggtitle(paste("Non-Linear: RMSE of IS estimator")) +
...
ggplot(data_ate_bias, aes(x=n,y=Bias, color=m) )+geom_point(size=5)+geom_line(size=2)+
  facet_grid(.~rho0)+
  theme_bw()+
  ggtitle("Non-Linear: Bias of IS estimator") +
  theme(plot.title = element_text(vjust = 2) ) +
  theme(plot.title = element_text(hjust = 0.5))+theme(plot.title = element_text(hjust = 0.5,size = 20, face = "bold"),
                                                      axis.text=element_text(size=14,face = "bold"),
                                                      axis.title.x=element_text(size=14,face="bold"),
...
ggplot(data_ate_sd, aes(x=n,y=SD, color=m) )+geom_point(size=5)+geom_line(size=2)+
  facet_grid(.~rho0)+
  theme_bw()+
  ggtitle("Non-Linear: SD of IS estimator") +
  theme(plot.title = element_text(vjust = 2)) +
  theme(plot.title = element_text(hjust = 0.5))+theme(plot.title = element_text(hjust = 0.5,size = 20, face = "bold"),
                                                      axis.text=element_text(size=14,face = "bold"),
                                                      axis.title.x=element_text(size=14,face="bold"),
...
write.csv(data_ate_mse,"result_IS_rmse_nonlinear.csv")
write.csv(data_ate_bias,"result_IS_bias_nonlinear.csv")
write.csv(data_ate_sd,"result_IS_SD_nonlinear.csv")
